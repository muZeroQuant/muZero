...
def _define_pack_kernel():
    triton, tl = _load_triton_modules()

    @triton.jit
    def _pack_q_values_kernel(
        q_ptr,
        packed_ptr,
        stride_q_row,
        stride_packed_row,
    ...
    return _pack_q_values_kernel


def triton_is_available() -> bool:
    try:
        _load_triton_modules()
    except Exception:
        return False
    return True


def logq_zeroanchor_pack_triton(q: torch.Tensor, bits: int) -> torch.Tensor:
    if not q.is_cuda:
        raise ValueError("`q` must be a CUDA tensor")
    if q.dtype != torch.uint8:
        raise ValueError(f"`q` must have dtype torch.uint8, but got {q.dtype}")
    if q.dim() != 2:
        raise ValueError(f"`q` must be rank-2, but got shape {tuple(q.shape)}")
    if bits <= 0 or bits > 8:
        raise ValueError(f"`bits` has to be in [1, 8], but got {bits}")

    q = q.contiguous()
    rows, group_size = q.shape
    q_bytes = (group_size * bits + 7) // 8
    packed_q = torch.empty((rows, q_bytes), dtype=torch.uint8, device=q.device)
    pack_kernel = _define_pack_kernel()
    pack_kernel[(rows,)](
        q,
        packed_q,
        q.stride(0),
        ...
        q_bytes,
        bits=bits,
        BLOCK_Q_BYTES=_next_power_of_two(q_bytes),
    )
    return packed_q


def logq_zeroanchor_quantize_triton(
    ...
    if grouped.dim() != 2:
        raise ValueError(f"`grouped` must be rank-2, but got shape {tuple(grouped.shape)}")
    if not grouped.is_cuda:
        raise ValueError("`grouped` must be a CUDA tensor")
    ...
    log_base = torch.log1p(torch.tensor(alpha, device=grouped.device, dtype=torch.float32))
    pos_companded = torch.log1p(alpha * pos_norm) / log_base
    neg_companded = torch.log1p(alpha * neg_norm) / log_base
    ...
    packed_q = logq_zeroanchor_pack_triton(q, bits)
